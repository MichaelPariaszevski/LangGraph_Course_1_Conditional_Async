...
workflow.add_node("b2", ReturnNodeValue("I am node: b2"))

workflow.add_edge(start_key="a", end_key="b")
workflow.add_edge(start_key="a", end_key="c")
workflow.add_edge(start_key="b", end_key="b2")
workflow.add_edge(start_key=["b2", "c"], end_key="d")

workflow.add_edge(start_key="d", end_key=END)

graph = workflow.compile()

graph.get_graph().draw_mermaid_png(output_file_path="parallel_fanout_graph.png")
